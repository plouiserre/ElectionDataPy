...
class CreatorCandidateDataTest(unittest.TestCase):
    def test_creator_candidate_data_factory_first_line(self) :        
        ...
        self.assertEqual("CROZET", model.candidate.last_name)
        self.assertEqual("Sylvie", model.candidate.first_name)
        self.assertEqual("F", model.candidate.sexe)
        self.assertEqual(1,model.candidate.party_id)
        self.assertEqual("Profession intermédiaire de la santé et du travail social", model.candidate.job)
        self.assertTrue(datetime.datetime(1962,6,3) == model.candidate.birth_date)
    ...
    def test_creator_candidate_data_factory_second_line(self):
        ...
        self.assertEqual("LEPEUPLE", model.candidate.last_name)
        self.assertEqual("Eric", model.candidate.first_name)
        self.assertEqual("M", model.candidate.sexe)
        self.assertEqual(10,model.candidate.party_id)
        self.assertEqual("Commerçant et assimilé", model.candidate.job)
        self.assertTrue(datetime.datetime(1971,11,5) == model.candidate.birth_date)
        self.assertEqual(False, model.candidate_is_sorting)
    ...
    def test_creator_candidate_data_factory_territoires_de_belfort(self):
        ...
        self.assertEqual("GRUDLER", model.candidate.last_name)
        self.assertEqual("Thiebaud", model.candidate.first_name)
        self.assertEqual("M", model.candidate.sexe)
        self.assertEqual(8,model.candidate.party_id)
        self.assertEqual("Personne diverse sans activité professionnelle de moins de 60 ans (sauf retraité)", model.candidate.job)
        self.assertTrue(datetime.datetime(1993,4,16) == model.candidate.birth_date)
        self.assertEqual(False, model.candidate_is_sorting)
    ...
    def test_creator_candidate_data_factory_cotes_d_or(self) : 
        ...
        self.assertEqual("MARTIN", model.candidate.last_name)
        self.assertEqual("Didier", model.candidate.first_name)
        self.assertEqual("M", model.candidate.sexe)
        self.assertEqual(8,model.candidate.party_id)
        self.assertEqual("Profession intermédiaire de la santé et du travail social", model.candidate.job)
        self.assertTrue(datetime.datetime(1956,8,20) == model.candidate.birth_date)
    ...
    def test_creator_candidate_data_factory_corsica(self):
        ...
        self.assertEqual("LIPPLER", model.candidate.last_name)
        self.assertEqual("Walter", model.candidate.first_name)
        self.assertEqual("M", model.candidate.sexe)
        self.assertEqual(14,model.candidate.party_id)
        self.assertEqual("Cadre de la fonction publique", model.candidate.job)
        self.assertTrue(datetime.datetime(1961,6,2) == model.candidate.birth_date)
        self.assertEqual(False, model.candidate_is_sorting)
    ...
    def test_creator_candidate_data_factory_guadeloupe(self):
        ...
        self.assertEqual("NABAJOTH", model.candidate.last_name)
        self.assertEqual("Alix", model.candidate.first_name)
        self.assertEqual("M", model.candidate.sexe)
        self.assertEqual(4,model.candidate.party_id)
        self.assertEqual("Cadre de la fonction publique", model.candidate.job)
        self.assertTrue(datetime.datetime(1973,11,11) == model.candidate.birth_date)
        self.assertEqual(False, model.candidate_is_sorting)
    ...
    def test_creator_candidate_data_factory_martinique(self):
        ...
        self.assertEqual("TABAR", model.candidate.last_name)
        self.assertEqual("Jonathan", model.candidate.first_name)
        self.assertEqual("M", model.candidate.sexe)
        self.assertEqual(13,model.candidate.party_id)
        self.assertEqual("Employé civil et agent de service de la fonction publique", model.candidate.job)
        self.assertTrue(datetime.datetime(1987,10,11) == model.candidate.birth_date)
        self.assertEqual(False, model.candidate_is_sorting)
    ...
    def test_creator_candidate_data_factory_guyane(self):
        ...
        self.assertEqual("MADÈRE", model.candidate.last_name)
        self.assertEqual("Christophe", model.candidate.first_name)
        self.assertEqual("M", model.candidate.sexe)
        self.assertEqual(4,model.candidate.party_id)
        self.assertEqual("Professeur, profession scientifique", model.candidate.job)
        self.assertTrue(datetime.datetime(1971,7,18) == model.candidate.birth_date)
        self.assertEqual(False, model.candidate_is_sorting)
    ...
    def test_creator_candidate_data_factory_reunion(self):
        ...
        self.assertEqual("SISTERON", model.candidate.last_name)
        self.assertEqual("Murielle", model.candidate.first_name)
        self.assertEqual("F", model.candidate.sexe)
        self.assertEqual(12,model.candidate.party_id)
        self.assertEqual("Profession libérale", model.candidate.job)
        self.assertTrue(datetime.datetime(1987,2,16) == model.candidate.birth_date)
        self.assertEqual(False, model.candidate_is_sorting)
    ...
    def test_creator_candidate_data_factory_mayotte(self):
        ...
        self.assertEqual("AOUNY", model.candidate.last_name)
        self.assertEqual("Yasmina", model.candidate.first_name)
        self.assertEqual("F", model.candidate.sexe)
        self.assertEqual(4,model.candidate.party_id)
        self.assertEqual("Professeur, profession scientifique", model.candidate.job)
        self.assertTrue(datetime.datetime(1986,3,31) == model.candidate.birth_date)
        self.assertEqual(False, model.candidate_is_sorting)
    ...
    def test_creator_candidate_data_factory_nouvelle_caledonie(self):
        ...
        self.assertEqual("GIL", model.candidate.last_name)
        self.assertEqual("Antoine", model.candidate.first_name)
        self.assertEqual("M", model.candidate.sexe)
        self.assertEqual(13,model.candidate.party_id)
        self.assertEqual("Policier et militaire", model.candidate.job)
        self.assertTrue(datetime.datetime(1956,5,26) == model.candidate.birth_date)
        self.assertEqual(False, model.candidate_is_sorting)
    ...
    def test_creator_candidate_data_factory_polynesie_francaise(self):
        ...
        self.assertEqual("HAITI", model.candidate.last_name)
        self.assertEqual("Pascale", model.candidate.first_name)
        self.assertEqual("F", model.candidate.sexe)
        self.assertEqual(13,model.candidate.party_id)
        self.assertEqual("Profession libérale", model.candidate.job)
        self.assertTrue(datetime.datetime(1972,4,3) == model.candidate.birth_date)
        self.assertEqual(False, model.candidate_is_sorting)
    ...
    def test_creator_candidate_data_factory_saint_pierre_et_miquelon(self):
        ...
        self.assertEqual("GASTON", model.candidate.last_name)
        self.assertEqual("Olivier", model.candidate.first_name)
        self.assertEqual("M", model.candidate.sexe)
        self.assertEqual(4,model.candidate.party_id)
        self.assertEqual("Cadre administratif et commercial d'entreprise", model.candidate.job)
        self.assertTrue(datetime.datetime(1981,6,30) == model.candidate.birth_date)
        self.assertEqual(False, model.candidate_is_sorting)
    ...
    def test_creator_candidate_data_factory_wallis_et_futuna(self):
        ...
        self.assertEqual("UGATAI", model.candidate.last_name)
        self.assertEqual("Sandrine Aimée", model.candidate.first_name)
        self.assertEqual("F", model.candidate.sexe)
        self.assertEqual(9,model.candidate.party_id)
        self.assertEqual("Cadre administratif et commercial d'entreprise", model.candidate.job)
        self.assertTrue(datetime.datetime(1973,6,23) == model.candidate.birth_date)
        self.assertEqual(False, model.candidate_is_sorting)
    ...
    def test_creator_candidate_data_factory_saint_martin_saint_barthelemy(self):
        ...
        self.assertEqual("GIBBS", model.candidate.last_name)
        self.assertEqual("Daniel", model.candidate.first_name)
        self.assertEqual("M", model.candidate.sexe)
        self.assertEqual(13,model.candidate.party_id)
        self.assertEqual("Profession libérale", model.candidate.job)
        self.assertTrue(datetime.datetime(1968,8,1) == model.candidate.birth_date)
        self.assertEqual(False, model.candidate_is_sorting)
    ...
    def test_creator_candidate_data_factory_french_strangers(self):
        ...
        self.assertEqual("GONDARD", model.candidate.last_name)
        self.assertEqual("Cécilia", model.candidate.first_name)
        self.assertEqual("F", model.candidate.sexe)
        self.assertEqual(3,model.candidate.party_id)
        self.assertEqual("Cadre administratif et commercial d'entreprise", model.candidate.job)
        self.assertTrue(datetime.datetime(1980,6,4) == model.candidate.birth_date)
        self.assertEqual(False, model.candidate_is_sorting)
